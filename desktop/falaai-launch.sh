#!/usr/bin/env bash
# PT-BR: Lançador do Fala A.I. (Linux) — sobe o servidor na porta 80 (HTTPS) e abre o app.
# EN:    Fala A.I. launcher (Linux) — starts the server on port 80 (HTTPS) and opens the app.
set -e
...
if ! curl -k -s -o /dev/null "https://localhost/api/health" 2>/dev/null; then
  ...
  if command -v pkexec >/dev/null 2>&1; then
    pkexec "$HERE/run.sh" --https >/tmp/falaai.log 2>&1 &
  else
    ...
    if command -v gnome-terminal >/dev/null 2>&1; then
      ...
      notify-send "Fala A.I." "Execute manualmente: sudo $HERE/run.sh --https"
      exit 1
    fi
  fi
