#!/usr/bin/env bash
# =============================================================================
# PT-BR: Lança uma nova versão do OpenLingo. Uso:  ./release.sh 1.0.1 "notas da versão"
...
# EN:    Ship a new OpenLingo version. Usage: ./release.sh 1.0.1 "release notes"
# =============================================================================
set -e
cd "$(dirname "$0")"

VER="$1"
NOTES="${2:-Nova versão do OpenLingo.}"
if [ -z "$VER" ]; then
...
gh release create "v$VER" --title "OpenLingo v$VER 🦜" --notes "$NOTES"
